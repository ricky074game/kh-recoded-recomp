...
public:
  QtScreenView(SoftwareRenderer *renderer, NDSMemory *memory,
               UiSnapshotCallback snapshotCallback, QWidget *parent = nullptr)
      : QWidget(parent), m_renderer(renderer), m_memory(memory),
  ...
  void RefreshFrames() {
    ...
    // The renderer packs pixels as (R<<24)|(G<<16)|(B<<8)|A in uint32_t.
    // On little-endian x86, this stores bytes in memory as [A, B, G, R].
    // QImage::Format_RGBA8888 expects byte order [R, G, B, A] in memory.
    // Byte-swap each pixel so the memory layout matches Qt's expectation.
    auto byteSwapPixels =
        [](std::array<uint32_t, kDSWidth * kDSHeight> &pixels) {
          for (auto &px : pixels) {
            px = ((px >> 24) & 0xFF) // old R byte -> byte 0 (lowest addr)
                 | (((px >> 16) & 0xFF) << 8) // old G byte -> byte 1
                 | (((px >> 8) & 0xFF) << 16) // old B byte -> byte 2
                 | ((px & 0xFF) << 24); // old A byte -> byte 3 (highest addr)
          }
        };
    byteSwapPixels(top_pixels);
    byteSwapPixels(bottom_pixels);

    m_topImage =
        QImage(reinterpret_cast<const uchar *>(top_pixels.data()), kDSWidth,
               kDSHeight, static_cast<int>(kDSWidth * sizeof(uint32_t)),
               QImage::Format_RGBA8888)
            .copy();
    m_bottomImage =
        QImage(reinterpret_cast<const uchar *>(bottom_pixels.data()), kDSWidth,
               kDSHeight, static_cast<int>(kDSWidth * sizeof(uint32_t)),
               QImage::Format_RGBA8888)
            .copy();
...
protected:
  void paintEvent(QPaintEvent *) override {
    ...
    const int widthFromHeight = static_cast<int>(
        std::lround(halfHeight * (static_cast<double>(kDSWidth) / kDSHeight)));
    const int screenWidth = std::min(content.width(), widthFromHeight);
    const int screenHeight = static_cast<int>(
        std::lround(screenWidth * (static_cast<double>(kDSHeight) / kDSWidth)));

    const int topX = content.x() + (content.width() - screenWidth) / 2;
    const int topY = content.y() + (halfHeight - screenHeight) / 2;
    const int bottomY =
        content.y() + halfHeight + gap + (halfHeight - screenHeight) / 2;
    ...
    painter.drawText(m_topRect.adjusted(0, -26, 0, -6),
                     Qt::AlignLeft | Qt::AlignVCenter,
                     QStringLiteral("TOP SCREEN"));
    painter.drawText(m_bottomRect.adjusted(0, -26, 0, -6),
                     Qt::AlignLeft | Qt::AlignVCenter,
                     QStringLiteral("BOTTOM SCREEN"));

    const float scale = (m_bottomRect.width() > 0)
                            ? (static_cast<float>(m_bottomRect.width()) /
                               static_cast<float>(kDSWidth))
                            : 1.0f;
    m_memory->GetInputManager().SetDisplayScale(scale, m_bottomRect.x(),
                                                m_bottomRect.y());
  }
  ...
  void mousePressEvent(QMouseEvent *event) override {
    ...
    if (m_bottomRect.contains(event->pos())) {
      m_touchActive = true;
      m_memory->GetInputManager().HandleTouchPoint(event->pos().x(),
                                                   event->pos().y(), true);
      event->accept();
      return;
    }
...
public:
  QtRuntimeWindow(SoftwareRenderer *renderer, NDSMemory *memory,
                  std::atomic<bool> *running,
                  std::atomic<uint64_t> *uiPresentCount,
                  UiSnapshotCallback snapshotCallback,
                  QWidget *parent = nullptr)
      : QMainWindow(parent), m_renderer(renderer), m_memory(memory),
        m_running(running), m_uiPresentCount(uiPresentCount),
        m_snapshotCallback(std::move(snapshotCallback)) {
    setWindowTitle(QStringLiteral("KH Re:coded - Qt Frontend"));
    setMinimumSize(1120, 980);

    m_screenView =
        new QtScreenView(m_renderer, m_memory, m_snapshotCallback, this);
...
private:
  void createMenus() {
    ...
    QAction *toggleModsAction =
        modsMenu->addAction(QStringLiteral("Show &Mod Manager"));
    toggleModsAction->setCheckable(true);
    toggleModsAction->setChecked(true);
    connect(toggleModsAction, &QAction::triggered, this,
            [this, toggleModsAction](bool checked) {
    ...
    QAction *keyboardAction =
        optionsMenu->addAction(QStringLiteral("&Keyboard Settings"));
    connect(keyboardAction, &QAction::triggered, this,
            [this]() { showKeyboardDialog(); });

    QMenu *viewMenu = menuBar()->addMenu(QStringLiteral("&View"));
    QAction *showStatusAction =
        viewMenu->addAction(QStringLiteral("Show &Status Bar"));
    ...
    QAction *snapshotAction =
        toolsMenu->addAction(QStringLiteral("Manual Debug Snapshot"));
  ...
  void createModsDock() {
    ...
    m_modDock->setAllowedAreas(Qt::LeftDockWidgetArea |
                               Qt::RightDockWidgetArea);
    m_modDock->setFeatures(QDockWidget::DockWidgetMovable |
    ...
    for (size_t i = 0; i < m_mods.size(); ++i) {
      ...
      auto *item =
          new QListWidgetItem(QString::fromStdString(mod.name), m_modList);
    ...
    connect(m_modList, &QListWidget::itemChanged, this,
            [this](QListWidgetItem *item) {
              const int row = item->data(Qt::UserRole).toInt();
              if (row >= 0 && row < static_cast<int>(m_mods.size())) {
                m_mods[static_cast<size_t>(row)].enabled =
  ...
  void createKeyboardDialog() {
    ...
    auto *label = new QLabel(
        QStringLiteral("Native keyboard mapping\n\n"
                       "A: Space\n"
                       "B: Z\n"
                       "Select: Shift\n"
                       "Start: Enter\n"
    ...
    auto *closeButton =
        new QPushButton(QStringLiteral("Close"), m_keyboardDialog);
    layout->addWidget(closeButton);
    connect(closeButton, &QPushButton::clicked, m_keyboardDialog,
            &QDialog::hide);
  }
  ...
  void rebuildModDetails() {
    if (m_modDetails == nullptr || m_selectedMod < 0 ||
        m_selectedMod >= static_cast<int>(m_mods.size())) {
      return;
    }

    const ModEntryUi &mod = m_mods[static_cast<size_t>(m_selectedMod)];
    QString text;
    text += QStringLiteral("<b>") + QString::fromStdString(mod.name) +
            QStringLiteral("</b><br>");
    text += QStringLiteral("Author: ") + QString::fromStdString(mod.author) +
            QStringLiteral("<br>");
    text += QStringLiteral("Enabled: ") +
            QString::fromLatin1(mod.enabled ? "Yes" : "No") +
    ...
    if (!mod.dependencies.empty()) {
      ...
      for (const std::string &dependency : mod.dependencies) {
        text += QStringLiteral("- ") + QString::fromStdString(dependency) +
                QStringLiteral("<br>");
      }
    ...
    if (!mod.options.empty()) {
      ...
      for (const ModOptionUi &option : mod.options) {
        text += QStringLiteral("- ") + QString::fromStdString(option.name) +
                QStringLiteral(": ") +
                QString::fromLatin1(option.enabled ? "On" : "Off") +
                QStringLiteral("<br>");
      }
    }
...
int RunQtFrontend(int argc, char *argv[], SoftwareRenderer *renderer,
                  NDSMemory *memory, std::atomic<bool> *running,
                  std::atomic<uint64_t> *uiPresentCount,
                  UiSnapshotCallback snapshotCallback) {
  QApplication app(argc, argv);
  app.setApplicationName(QStringLiteral("KH Re:coded Recomp"));

  QtRuntimeWindow window(renderer, memory, running, uiPresentCount,
                         std::move(snapshotCallback));
  window.show();
  return app.exec();
}
