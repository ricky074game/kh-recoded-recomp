...
Status: implemented in the repo.
...
- [x] **Step 1: Clone vcpkg into the root**
```bash
git clone https://github.com/microsoft/vcpkg.git vcpkg
```

- [x] **Step 2: Bootstrap vcpkg**
```bash
...
- [x] **Step 1: Create vcpkg.json at the root**
```json
{
...
- [x] **Step 2: Update root CMakeLists.txt to use vcpkg**
```cmake
set(CMAKE_TOOLCHAIN_FILE "${CMAKE_CURRENT_SOURCE_DIR}/vcpkg/scripts/buildsystems/vcpkg.cmake" CACHE STRING "")
project(kh_recoded_recomp)
```

- [x] **Step 3: Update runtime/CMakeLists.txt to find Qt6 instead of SDL2**
...
- [x] **Step 1: Replace SDL includes and types in hw_input.h**
Replace `#include <SDL2/SDL.h>` with `#include <QKeyEvent>` and update method signatures.

- [x] **Step 2: Update mappings in hw_input.cpp**
...
- [x] **Step 1: Remove SDL dependencies from Software Renderer**
Change `SDL_Texture*` to `std::vector<uint32_t>` or raw buffers. Remove `Present` method that used `SDL_RenderCopy`.

- [x] **Step 2: Update CMakeLists.txt with renamed files**
...
- [x] **Step 1: Create KeyboardSettingsDialog**
Use `QDialog` with `QTableWidget` or labels for key config.

- [x] **Step 2: Create ModManagerDialog**
